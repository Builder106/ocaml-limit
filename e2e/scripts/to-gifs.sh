...
#
# Source mp4s are recorded at 1440x900 (see playwright.demo.config.ts).
# Output GIFs are 1280px wide so they slot crisply into GitHub's
# ~890 CSS-px README column on Retina without being upscaled by the
# browser, and stay under GitHub's 10 MB inline-image limit at the
# bitrates we're using.
#
...
for mp4 in "$SRC_DIR"/*.mp4; do
  ...
  ffmpeg -y -i "$mp4" -i "$palette" \
    -lavfi "fps=10,scale=1280:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=sierra2_4a" \
    "$out" >/dev/null 2>&1
  rm -f "$palette"
  trap - EXIT
