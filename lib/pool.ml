(** {1 Pre-Allocated Object Pool}
    ...
    Eliminates per-order heap allocation in the matching hot path by maintaining a
    free-list over a pre-allocated array of queue nodes.

    {b Design rationale}: In a GC'd language, the primary source of latency jitter is
    minor-heap promotion and subsequent major GC pauses. By pre-allocating all node
    storage at initialization time and recycling slots via an index-based free-list, the
    matching engine's inner loop triggers zero GC minor collections.

    The pool uses a simple stack-based free-list: freed indices are pushed onto
    [free_stack] and popped on allocation. This gives O(1) alloc/free with no heap
    allocation. *)
...
let create capacity =
  ...
  for i = 0 to capacity - 1 do
    nodes.(i) <- { qn_order = sentinel_order; qn_next = -1; qn_prev = -1 }
  done;
...
(** [utilization pool] returns the fraction of the pool currently in use. Used for
    capacity monitoring. *)
let utilization pool = Float.of_int pool.allocated /. Float.of_int pool.capacity
