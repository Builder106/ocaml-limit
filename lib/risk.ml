(** {1 Pre-Trade Risk Engine}
    ...
    matching engine. These checks run in the hot path and are therefore designed for
    minimal overhead — no allocation, no branching on heap-allocated data, pure integer
    arithmetic.
...
(** Risk check result — uses a variant rather than exceptions to avoid the overhead of
    exception handling in the hot path. The [Ok] case carries no payload to avoid
    allocation. *)
...
(** [check_price_band config order] validates that the order price is within the
    ...
    {b Rationale}: Fat-finger errors (e.g., buying at $10,000 instead of $100.00) are a
    major source of risk in electronic trading. This check catches erroneous prices before
    they can cause a catastrophic fill.
...
(** [check_position_limit config tracker order] validates that filling this order would
    ...
    This is a pessimistic check — it assumes the entire order will be filled immediately.
    In practice, partial fills would consume less position capacity, but the conservative
    approach is standard practice in production risk systems.
...
(** [validate config tracker order] runs the full pre-trade risk check pipeline. Returns
    ...
    The checks are ordered by computational cost (cheapest first) to maximize early-exit
    probability.
