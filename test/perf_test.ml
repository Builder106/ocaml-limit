(** {1 Performance Regression Tests}
    ...
    Locks current engine perf characteristics so a refactor can't
    ...
    Thresholds are intentionally generous so this passes on under-
    provisioned CI runners. Tighten them as the engine improves.

    The allocation guard uses [Gc.minor_words] (cumulative words
    allocated in the minor heap), NOT [Gc.minor_collections] —
    [Gc.stat] itself allocates a 17-field record and triggers minor
    flushes, so collection counts reflect measurement overhead, not
    engine work. With [minor_words] the engine reports ~792 words
    fixed (cold-path price-level records on first sight of each
    price) and 0 words per submit at scale. *)
...
let p99_ceiling_us = 100.0          (* engine hot path is sub-μs; 100μs swallows gettimeofday noise *)
...
let test_throughput_floor () =
    ...
    Alcotest.(check bool)
        (Printf.sprintf "throughput >= %.2f Mops/s (measured %.2f)"
            throughput_floor_mops mops_per_sec)
        true (mops_per_sec >= throughput_floor_mops)
