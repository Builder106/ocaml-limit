...
let test_risk () =
  ...
  Alcotest.(check bool)
    "reject low" true
    (Risk.check_price_band config low_order = Risk.Reject_price_band);
  Alcotest.(check bool)
    "reject high" true
    (Risk.check_price_band config high_order = Risk.Reject_price_band);
  Alcotest.(check bool)
    "pass low" true
    (Risk.check_price_band config good_order = Risk.Pass);

  (* Max qty *)
  let big_order = { good_order with remaining_qty = 100 } in
  Alcotest.(check bool)
    "reject big" true
    (Risk.check_max_qty config big_order = Risk.Reject_max_qty);

  (* Position limit *)
  let over_limit = { good_order with remaining_qty = 25 } in
  Alcotest.(check bool)
    "reject pos" true
    (Risk.check_position_limit config tracker over_limit = Risk.Reject_position_limit);
  Alcotest.(check bool)
    "pass pos" true
    (Risk.check_position_limit config tracker good_order = Risk.Pass);

  (* Ask side position limit (requires net_position < 0 eventually) *)
  let ask_order = { over_limit with side = Types.Ask } in
  Alcotest.(check bool)
    "reject ask pos" true
    (Risk.check_position_limit config tracker ask_order = Risk.Reject_position_limit);

  (* Full validate pipeline *)
  Alcotest.(check bool)
    "val max_qty" true
    (Risk.validate config tracker big_order = Risk.Reject_max_qty);
  Alcotest.(check bool)
    "val price" true
    (Risk.validate config tracker high_order = Risk.Reject_price_band);
  Alcotest.(check bool)
    "val pos" true
    (Risk.validate config tracker over_limit = Risk.Reject_position_limit);
  Alcotest.(check bool)
    "val pass" true
    (Risk.validate config tracker good_order = Risk.Pass);

  (* String repr *)
  Alcotest.(check string) "str pass" "PASS" (Risk.risk_result_to_string Risk.Pass);
  Alcotest.(check string)
    "str pb" "REJECTED: price outside configured band"
    (Risk.risk_result_to_string Risk.Reject_price_band);
  Alcotest.(check string)
    "str mq" "REJECTED: quantity exceeds maximum"
    (Risk.risk_result_to_string Risk.Reject_max_qty);
  Alcotest.(check string)
    "str pl" "REJECTED: would breach position limit"
    (Risk.risk_result_to_string Risk.Reject_position_limit)

let test_price_index () =
  let idx = Price_index.create 4 ~empty_value:0 in
  (* 2^2 capacity *)
  ...
  with Not_found -> (
    ...
    (try
       let _ = Price_index.find idx2 100 in
       ()
     with Not_found -> ());
...
let test_stats () =
  ...
  st.Stats.start_time <- Unix.gettimeofday () +. 10.0;
  (* Force elapsed <= 0 *)
